...
c     de cada planeta
    dimension raios(11:19),raiosatual(11:19,2),velocidades(11:19)
...
c                 realiza um interacao em cada coordenada
                xproximo = 2.0d0*raiosatual(i,1) - xantigo - gms*raios
   1atual(i,1)/(raio**3.0d0)*(deltat**2.0d0)
                yproximo = 2.0d0*raiosatual(i,2) - yantigo - gms*raios
   1atual(i,2)/(raio**3.0d0)*(deltat**2.0d0)

c                 salva os valores das coordenadas antes de altera-los
                xantigo = raiosatual(i,1)
                yantigo = raiosatual(i,2)

c                 atualiza os valores das coordenadas
                raiosatual(i,1) = xproximo
                raiosatual(i,2) = yproximo

c                  write(*,*)raiosatual(i,1),raiosatual(i,2)
